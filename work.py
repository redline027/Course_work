...
def getXY(event):
    src.append([float(event.x-2), float(event.y-2)])

src = []
window.bind('<ButtonPress>', getXY)
window.mainloop()
...
def residual(params, data):
    ...
    model = F(angle_fit, translation_fit)
    return model - data
...
def write(params, cor, f):
    ...
    f.write('Mean error:' + str(np.mean(residual(params, data))) + '\n')
